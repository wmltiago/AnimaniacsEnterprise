...
try{
#COMEÇO DO IF
$stmt = $conexao->prepare("insert into usuario (nome, telefone, cpf,rg, endereco, data_nascimento, sexo, email, senha, login) values(?,?,?,?,?,?,?,?,?,?)");

$nome=$_POST["nome"]; $telefone=$_POST["telefone"]; $cpf=$_POST["cpf"]; $rg=$_POST["rg"]; $endereco=$_POST["endereco"]; $data_nascimento=$_POST["data_nascimento"];$sexo=$_POST["sexo"];$email=$_POST["email"];$senha=$_POST["senha"];$login=$_POST["login"];
